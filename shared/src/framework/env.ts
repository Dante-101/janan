...
export const isStage: boolean = process.env.NODE_ENV == 'production' && !!process.env.STAGE_ENV
export const isTest: boolean = process.env.NODE_ENV == 'test'
export const isInternal: boolean = isDev || isStage

export const isProd: boolean = process.env.NODE_ENV == 'production' && !isInternal

export const isDemo = !!process.env.DEMO_ENV
...
if (!isProd && !isDev && !isTest && !isStage) {
    //Env should not import anything. Throw an error if ENV is not set
    throw new Error(`The application has to run in either development, production or test mode. INVALID_CONFIG`)
}