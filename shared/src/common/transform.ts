...
export const secondsToTimeStr = (time: number, format: string = "hh:mm:ss"): string => {
    time = Math.round(time)
    const secs = time % 60
    const mins = ((time - secs) / 60) % 60
    const hours = ((time - secs - mins * 60) / 3600)
    const hh = hours > 0 ? toAtLeast2DigitStr(hours) + ":" : ""
    const h = hours > 0 ? hours + ":" : ""
    const mm = toAtLeast2DigitStr(mins)
    const ss = toAtLeast2DigitStr(secs)
    return format.replace(/hh:?/, hh).replace(/mm:?/, mm).replace("ss", ss).replace(/h:?/, h)
        .replace(/m:?/, mins.toString()).replace("s", secs.toString())
}

export const shortenTxt = (txt: string | object, length: number = 300) => {
...
export const toArr = <T>(obj: T | T[]): T[] => Array.isArray(obj) ? obj : [obj]
export const toNonEmptyArr = <T>(obj: T | T[]): T[] => toArr(obj).filter(v => v)
...
export const b64ToString = (str: string) => Buffer.from(str, 'base64').toString('binary')
export const stringTob64 = (str: string) => Buffer.from(str).toString('base64')